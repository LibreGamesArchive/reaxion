...
/**
 * Input Handler for the allows player to move it forward, backward, left, and
 * right. Attacks and other inputs pertaining to the player will also go here.
 * 
 * @author Khoa
 * 
 */
public class ClientPlayerInput extends PlayerInput {
	...
	private boolean jumpOn = false, jumping = false; // jumping is for server
														// only
	 ...
	 * 
	 * @param b
	 *            the current BattleGameState
	 * @param q
	 *            the array of classes of attacks
	 */
	 ...
	 * based on if certain keys are pressed (numpad).
	 * 
	 * @author Khoa
	 */
	...
	/**
	 * Must be called during {@code update()} by the {@code GameState}. Checks
	 * the current state of input commands, preserving priority by the order in
	 * which they are pressed. Sets player's unit vector accordingly. (Since
	 * theta = 0 is along the Z axis,)
	 * 
	 * @author Khoa
	 */
	public void checkKeys() {
		...
		jumpOn = keyboard.isValidCommand("jump", true);
		...
		if (Math.abs(unitX) + Math.abs(unitY) + Math.abs(unitZ) > 1) {
			float hyp = (float) Math.sqrt(Math.pow(unitX, 2) + Math.pow(unitY, 2)
					+ Math.pow(unitZ, 2));
			unitX /= hyp;
			unitY /= hyp;
			unitZ /= hyp;
	 ...
	 */
	/*
	 * private void executeAttack(int ind) { if (!player.flinching &&
	 * player.currentAttack == null) { try { if (attacks[ind] != null) {
	 * Character[] friends = new Character[1]; friends[0] = partner;
	 * attacks[ind].getConstructors()[1].newInstance(new AttackData(player,
	 * friends, state.getTarget())); } } catch (Exception e) {
	 * System.out.println
	 * ("Fatal error: Attack array parameter was not an Attack.");
	 * e.printStackTrace(); } } }
	 */
