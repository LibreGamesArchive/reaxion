...
public abstract class SmartAI extends AIInput {
     ...
     */
    protected Vector3f sidestep(StageGameState state, ArrayList<Model> m, Vector3f bias) {
    	// TODO: Implement checking over varying theta values, preferably not by brute force.
    	
    	Vector3f step = null;
    	Vector3f t = character.model.getWorldTranslation();
    	
    	// TODO: Find product of distances based on iterated angle changes
    	float distProd = 1;
    	for (int i=0; i<m.size(); i++) {
    		...
    		if (ind >= 0) {
    			...
    			float dist = ((t.x - prevPos.x)*(pos.x - prevPos.x) + (t.y - prevPos.y)*(pos.y - prevPos.y)) / pos.distanceSquared(prevPos);
    			
    			distProd *= dist;
    		}
    	}
    	
    	// TODO: Compare vector similarities to bias vector.
    	float similarity = 0;
     ...
     */
    protected ArrayList<Model> checkJump(StageGameState state, float velocityY, float precision) {
    	...
    	for (int i=0; i<m.size(); i++) {
    		...
    		if (ind >= 0) {
    			...
    			float angleX = Math.abs((FastMath.atan2(pos.y-t.y, pos.x-t.x) - FastMath.atan2(threat.y-apex.y, threat.x-apex.x) + 4*FastMath.PI)
    					%(2*FastMath.PI));
    			float angleZ = Math.abs((FastMath.atan2(pos.y-t.y, pos.z-t.z) - FastMath.atan2(threat.y-apex.y, threat.z-apex.z) + 4*FastMath.PI)
    					%(2*FastMath.PI));

    			// evaluate angle based on precision
    			if ((angleX + angleZ)/2 >= 1.57f * precision)
    				list.add(o);
    		}
    	}
     ...
     * @return A list of objects flagged as threats.
     */
    protected ArrayList<Model> findThreats(StageGameState state, float radius, float precision) {
    	ArrayList<Model> list = new ArrayList<Model>();
    	Vector3f t = character.model.getWorldTranslation();
    	ArrayList<Model> m = state.getModels();
        		...
        		if (m.get(i).equals(pm.get(j))) {
        			...
        			if (radius < 0 || o.model.getWorldTranslation().distance(t) <= radius) {
        				...
        				if ((o.damagePerFrame > 0 || (characters && o instanceof Character)) && (o.users== null || !o.users.contains(character))) {
        					...
        					// check if object is approaching if immediate is flagged
        					if (!immediate || o.model.getWorldTranslation().distance(t) < p.distance(t)) {
