...
/**
 * {@code StageSelectionState} is designated to the stage selection menu. It contains a instance of {@code StageSelectionOverlay}.
 * This state adds key functionality to the stage selection menu. The arrow keys are used to change the selected stage name,
 * and the enter key is used to load that stage and create a new {@code BattleGameState}.
 * @author Brian
 *
 */
...
//		Quad cover = new Quad("cover", DisplaySystem.getDisplaySystem().getWidth(), DisplaySystem.getDisplaySystem().getHeight());
//		cover.setSolidColor(new ColorRGBA(0, 0, 0, 1));
//		BitmapText loadingText = new BitmapText(stageSelectionNode.getTextFont(), false);
//		loadingText.setText("Loading " + stageSelectionNode.getSelectedStageName());
//		loadingText.setSize(48);
//		loadingText.setLocalTranslation(cover.getWidth() / 2, cover.getHeight() / 2, 0);
//		
...
//	}
	...
	/**
	 * Switches from {@code StageSelectionState} to {@code BattleGameState} and passes a {@code Stage} object corresponding
	 * to the stage the user selects in the menu.
	 */
