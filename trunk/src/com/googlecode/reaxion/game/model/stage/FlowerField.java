...
public class FlowerField extends Stage {
    ...
    public void act(BattleGameState b) {
    	...
    	if (count == 4) {
    		...
    		petal.setVelocity(new Vector3f(FastMath.nextRandomFloat()-.3f, 1, FastMath.nextRandomFloat()-.2f));
    		count = 0;
    ...
    public Point2D.Float snapToBounds(Point2D.Float center) {
    	...
    	float dist = FastMath.sqrt(FastMath.pow(center.x, 2) + FastMath.pow(center.y, 2));
    ...
    public Point2D.Float[] bound(Point2D.Float center, float radius) {
    	...
    	if (dist + radius > r) {
    		...
    		if (dist - radius == r || dist + radius == r) {
    			...
    			p.add(new Point2D.Float(r*FastMath.cos(angle), r*FastMath.sin(angle)));
    		} else {
    			// add the two points of collision
    			float ptY = FastMath.sqrt(FastMath.pow(radius, 2) - FastMath.pow(r - dist, 2));
    			float minor = FastMath.atan2(ptY, r - dist);
    			p.add(new Point2D.Float(r*FastMath.cos(angle) + radius*FastMath.cos(angle+minor),
    					r*FastMath.sin(angle) + radius*FastMath.sin(angle+minor)));
    			p.add(new Point2D.Float(r*FastMath.cos(angle) + radius*FastMath.cos(angle-minor),
    					r*FastMath.sin(angle) + radius*FastMath.sin(angle-minor)));
    		}
    ...
    public LightState createLights() {
        ...
        for (float a=0; a<2*FastMath.PI; a+=2*FastMath.PI/8) {
        	...
        	p.setLocation( new Vector3f( 600*FastMath.cos(a), 0, 600*FastMath.sin(a) ) );
        	p.setEnabled( true );
        	lightState.attach( p );
        }
