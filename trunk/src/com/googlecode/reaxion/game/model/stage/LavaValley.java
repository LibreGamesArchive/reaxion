...
/**
 * A primordial setting where ancient battles were held.
 */
public class LavaValley extends Stage {
	...
	private final float angleInc = FastMath.PI / 720;
	...
	public void loadComponents(StageGameState b) {
		sky = LoadingQueue.quickLoad(new Model("stages/lava-valley_sky"), null);
		model.attachChild(sky.model);
		lava = LoadingQueue.quickLoad(new Model("stages/lava-valley_lava"), null);
		model.attachChild(lava.model);
	}
	...
	public void act(StageGameState b) {
		...
		lava.rotate(new Vector3f(FastMath.sin(angle), 0, FastMath.cos(angle)));
	}
